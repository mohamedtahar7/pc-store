...
export default function BuildsPage() {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((p) => {
      // Force filter to only show products in the "Builds" category
      const isBuild = p.category === "Builds";
      const matchesSearch = p.name
        .toLowerCase()
        .includes(searchQuery.toLowerCase());
      const matchesPrice =
        p.price >= priceRange.min && p.price <= priceRange.max;
      return isBuild && matchesSearch && matchesPrice;
    });
  }, [searchQuery, priceRange]);
      ...
      <div className="max-w-7xl mx-auto px-6">
        <header className="mb-12">
          <h1 className="text-4xl font-black text-slate-900 tracking-tight mb-4">
            Browse <span className="text-blue-600 italic">Builds</span>
          </h1>
          <p className="text-slate-500 font-medium max-w-xl">
            Professional workstations and precision hardware for 2026.
          </p>
        </header>
